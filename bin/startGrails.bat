...
set CLASS=org.codehaus.groovy.grails.cli.support.GrailsStarter
...
set AGENT_STRING=-javaagent:%GRAILS_HOME:\=/%/lib/org.springframework/springloaded/jars/springloaded-1.2.7.RELEASE.jar -Xverify:none -Dspringloaded.synchronize=true -Djdk.reflect.allowGetCallerClass=true -Dspringloaded=\"%SPRINGLOADED_PARAMS%\"
set DISABLE_RELOADING=
if "%GRAILS_OPTS%" == "" (
	set GRAILS_OPTS=-server -Xmx1024M -Xms512M -Dfile.encoding=UTF-8
	if not "%GRAILS_NO_PERMGEN%" == "1" (
		set GRAILS_OPTS=-server -Xmx1024M -Xms512M -XX:PermSize=512m -XX:MaxPermSize=1024m -Dfile.encoding=UTF-8
	)
)
...
set STARTER_CLASSPATH=%GRAILS_HOME%\lib\org.codehaus.groovy\groovy-all\jars\groovy-all-2.4.10.jar;%GRAILS_HOME%\dist\grails-bootstrap-2.5.6.jar
